...
### **Read [MIGRATION.md](MIGRATION.md) before upgrading.** Beta release; published under npm's `beta` dist-tag. Existing users on `latest` stay on V17.4.13 and will not auto-upgrade. Opt in with `npm install node-red-contrib-telegrambot@beta`.
